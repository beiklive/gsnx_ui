...
void BoxPage::Build(Widget* host, UiContext& ui) {
    ...
    first->SetSize(0.0f, 117.9f);

    Box* radius0 = helpers::Tile(first, "radius 0", 117.0f, 87.4f, 0.0f);
    (void)radius0;

    Box* radius8 = helpers::Tile(first, "radius 8", 117.0f, 87.4f, Theme::kRadius);
    (void)radius8;

    Box* radius20 = helpers::Tile(first, "radius 20", 117.0f, 87.4f, 20.0f);
    (void)radius20;

    // 四角独立 + 边框 + 阴影
    Box* mixed = helpers::Tile(first, "四角独立 + shadow", 148.2f, 87.4f, 0.0f, Theme::kBgWidget, Theme::kBorder);
    mixed->SetRadius(22.0f, 4.0f, 22.0f, 4.0f);
    mixed->SetBorder(2.0f, Theme::kOrange);
    mixed->shadow = ShadowStyle::Soft(26.0f);
    mixed->shadow.offset = ImVec2(0.0f, 6.2f);
    mixed->shadow.color = Theme::kShadow;

    // 第二行：溢出 + 可聚焦 Box
    Box* second = helpers::Row(host, 14.0f);
    second->SetSize(0.0f, 112.1f);

    overflow_hidden_ = helpers::Tile(second, "overflow: hidden", 171.6f, 84.2f, Theme::kRadiusSmall, Theme::kBgEditor,
                                     Theme::kBorderStrong);
    overflow_hidden_->overflow = Overflow::Hidden;
    overflow_hidden_->layout = LayoutMode::Vertical;
    overflow_hidden_->align_x = Align::Start;
    overflow_hidden_->padding = EdgeInsets::All(7.8f);
    for (int i = 0; i < 6; ++i) {
        overflow_hidden_->AddLabel("超出这一行的内容会被裁剪 " + std::to_string(i), Theme::kFontSmall,
                                   Theme::kTextMuted);
    }

    Box* overflow_scroll = helpers::Tile(second, "", 171.6f, 84.2f, Theme::kRadiusSmall, Theme::kBgEditor,
                                         Theme::kBorderStrong);
    overflow_scroll->overflow = Overflow::Scroll;
    overflow_scroll->layout = LayoutMode::Vertical;
    overflow_scroll->gap = ImVec2(0.0f, 4.7f);
    overflow_scroll->align_x = Align::Stretch;
    overflow_scroll->padding = EdgeInsets::All(7.8f);
    ...
    focus_card_ = helpers::Tile(second, "焦点：缩放 + 位移 + 焦点框", 184.1f, 84.2f, Theme::kRadius,
    ...
    focus_card_->focus_frame_offset = 4.7f;
