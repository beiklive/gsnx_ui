...
| 点未选中的卡 | 选中它（并把它滚到中心） |
| 点已选中的卡 | 启动（= A） |
| 横向拖动 | 滚卡 + **松手带甩动惯性**（速度够大时继续滑，摩擦衰减、快停再吸附），**选中项跟着手指走**：按下时是第 i 张，拖过 k 张卡宽就选中第 i+k 张；松手把它吸附到行中心（不会弹回按下时的焦点） |
| 滚轮 | 横向滚卡 |

甩动（惯性）与容器滚动同一套参数：`Style::fling_threshold`（默认 320 px/s，低于它就纯吸附）、
`Style::fling_max`（4200 px/s）、`Style::friction`（5.0 /s）；甩动结束或点住时都会 `SyncIndexToDrag()`
把选中对到停下来的那张。实测：快速拖 300px 时选中停在 2，松手后甩到 4。

拖动为什么不按「谁在行中心」算选中：一行里同时可见好几张卡，行中心那张在按下瞬间往往不是当前选中的那张
（比如焦点在第 1 张、行中心落在第 3 张），一按下去选中就会跳。所以选中 = **按下时的下标 + 位移换算出的卡数**。
拖动期间置 `Global::pointer_dragging = true`：基类据此取消这次点击，也不会带动外层页面纵向滚。
